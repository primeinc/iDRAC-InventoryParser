...
def parse_hardware_info(file_path, interested_attributes=None):
    ...
    for child in root:
        ...
        if interested_attributes is not None and component_name not in interested_attributes:
            continue
        ...
        for sub_child in child:
            ...
            if interested_attributes is None or (
                component_name in interested_attributes and 
                any(re.match(pattern, key) and attribute_name in interested_attributes[component_name].get(pattern, [])
                ...
                for sub_sub_child in sub_child:
                    if sub_sub_child.tag == "DisplayValue":
                        hardware_info[component_name][key][attribute_name] = sub_sub_child.text
                        
                        # Special case to calculate SizeInGigabytes for Disk.Bay.*
                        if component_name == 'DCIM_PhysicalDiskView' and attribute_name == 'SizeInBytes':
                            try:
                                size_in_bytes = int(sub_sub_child.text.split(" ")[0])
                                size_in_gb = size_in_bytes / (1024 ** 3)  # Bytes to Gigabytes
                                hardware_info[component_name][key]['SizeInGigabytes'] = f"{size_in_gb} GB"
                            except ValueError:
                                hardware_info[component_name][key]['SizeInGigabytes'] = 'Conversion Error'
                                    
    return hardware_info

def unique_hardware_info(hardware_info, comparison_attributes, interested_attributes=None):
    ...
    for component, keys in hardware_info.items():
        if component not in comparison_attributes and (interested_attributes is None or component not in interested_attributes):
        ...
        for key, attributes in keys.items():
            ...
            if component == 'DCIM_NICView' and 'ProductName' in comparison_data:
                comparison_data['ProductName'] = re.sub(r' - [A-F0-9:]+$', '', comparison_data['ProductName'])
...
def generate_md_table(hardware_info, type='all'):
    ...
    if type == 'unique':
        for component, unique_components in hardware_info.items():
            ...
            headers = ['Count'] + [key for key, _ in next(iter(unique_components.keys()))]
            md_table = [f"### {component}", "| " + " | ".join(headers) + " |", "| " + " :--: |" * len(headers) ]
        ...
        for component, keys in hardware_info.items():
            ...
            md_table = [f"### {component}", "| Key | " + " | ".join(headers) + " |", "| :--: | " + " :--: |" * len(headers)]

            # Generate Markdown table rows
            for key, attributes in keys.items():
                row = [key] + [str(attributes.get(header, 'N/A')) for header in headers]  # Convert None to 'N/A' and everything to string
                md_table.append("| " + " | ".join(row) + " |")
    ...
    if type == 'unique':
        ...
        for component, keys in hardware_info.items():
            for key, attributes in keys.items():
                ...
                md_table.append(f"| {info_str} | 1 |")  # Assuming each entry counts as 1
...
# def generate_consolidated_md_table(summarized_hardware_info):
#     # Extract system information
#     system_info = summarized_hardware_info.get('DCIM_SystemView', {})
#     system_info_tuple = next(iter(system_info.keys()), {})
#     system_info = dict(system_info_tuple)
#     model = system_info.get('Model', 'Unknown')
#     service_tag = system_info.get('ServiceTag', 'Unknown')
...
#     # Generate Markdown table rows
#     for component, unique_components in summarized_hardware_info.items():
#         for comparison_tuple, count in unique_components.items():
#             comparison_dict = dict(comparison_tuple)
            
#             # Generate Info string based on component type
#             if component == 'DCIM_MemoryView':
#                 info_str = f"{comparison_dict.get('Model', '')} - {comparison_dict.get('PartNumber', '')} - {comparison_dict.get('Speed', '')} - {comparison_dict.get('Size', '')}"
#             elif component == 'DCIM_PhysicalDiskView':
#                 info_str = f"{comparison_dict.get('MediaType', '')} - {comparison_dict.get('Model', '')} - {comparison_dict.get('SizeInGigabytes', '')}"
#             elif component == 'DCIM_CPUView':
#                 info_str = comparison_dict.get('Model', '')
#             elif component == 'DCIM_PowerSupplyView':
#                 info_str = comparison_dict.get('Model', '').strip()
#             elif component == 'DCIM_SystemView':
#                 # We've already used this for the table title, so skip
#                 continue
#             else:
#                 info_str = comparison_dict.get('ProductName', '')
...
if __name__ == '__main__':
    ...
    parser.add_argument('output_type', choices=['raw', 'md'], help='Output type: raw or Markdown.')
    parser.add_argument('--filtered', action='store_true', help='Apply attribute filtering.')
    parser.add_argument('--unique', action='store_true', help='Output the flattened unique hardware configuration.')
    parser.add_argument('--pretty', action='store_true', help='For MD table to bet better formatted. Implies --unique.')
    # Parse arguments
    ...
    filtered_attributes = {
        ...
        'DCIM_SystemView': {
            'System.Embedded.1': ['BoardPartNumber', 'BoardSerialNumber', 'AssetTag', 'HostName',
                                'BIOSReleaseDate', 'BIOSVersionString', 'ChassisServiceTag',
                                'ExpressServiceCode', 'ServiceTag', 'Manufacturer', 'Model',
                                'LifecycleControllerVersion', 'SystemGeneration']
        },
        'DCIM_NICView': {
            'NIC.Integrated.*': ['Protocol', 'MediaType', 'VendorName', 'PermanentMACAddress', 'CurrentMACAddress', 'ProductName', 'DeviceDescription']
        },
        'DCIM_PowerSupplyView': {
            'PSU.Slot.*': ['TotalOutputPower', 'Manufacturer', 'PartNumber', 'SerialNumber', 'Model', 'DeviceDescription']
        },
        'DCIM_PhysicalDiskView': {
            'Disk.Bay.*': ['DriveFormFactor', 'MaxCapableSpeed', 'MediaType', 'BlockSizeInBytes', 'BusProtocol', 'SerialNumber', 'Revision', 'Model', 'SizeInBytes', 'Slot', 'Connector', 'RaidStatus', 'DeviceDescription'],
            'Disk.Direct.*': ['DriveFormFactor', 'MaxCapableSpeed', 'MediaType', 'BlockSizeInBytes', 'BusProtocol', 'SerialNumber', 'Revision', 'Model', 'SizeInBytes', 'Slot', 'Connector', 'RaidStatus', 'DeviceDescription']

        },
        'DCIM_EnclosureView': {
            'Enclosure.Internal.0-1:RAID.Integrated.1-1': ['ProductName', 'SlotCount', 'Version', 'DeviceDescription']
        },
        'DCIM_CPUView': {
            'CPU.Socket.*': ['Cache3Size', 'Cache2Size', 'Cache1Size', 'Model', 'Manufacturer', 'TurboModeCapable', 'VirtualizationTechnologyCapable', 'HyperThreadingCapable', 'Characteristics', 'NumberOfProcessorCores', 'NumberOfEnabledThreads', 'NumberOfEnabledCores', 'MaxClockSpeed', 'CPUFamily', 'DeviceDescription']
        }
    }
    ...
    hardware_info = parse_hardware_info(args.file_path, filtered_attributes if args.filtered else None)

    if args.unique:
        hardware_info = unique_hardware_info(hardware_info, unique_comparison_attributes)
    ...
    # print(generate_md_table(summarized_hardware_info))


# print(generate_consolidated_md_table(summarized_hardware_info))